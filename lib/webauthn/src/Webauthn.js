...
class Webauthn {
  constructor(options = {}) {
    ...
    if (Array.isArray(this.config.userFields)) {
      // Ensure mapping
      this.config.userFields = this.config.userFields.reduce(
        (state, current) => {
  ...
  response(options = {}) {
    ...
    return async (req, res, next) => {
      ...
      if (type !== "public-key") {
        return res
          .status(400)
          .json({ message: "response type must be 'public-key'" });
      }
      ...
      try {
        ...
        return res
          .status(400)
          .json({ message: "failed to decode client data" });
      }
      ...
      try {
        if (response.attestationObject !== undefined) {
          ...
          console.log("verifyAuthenticatorAttestationResponse", result);
          if (result.verified) {
          ...
          console.log("verifyAuthenticatorAssertionResponse", result);
          if (result.verified) {
          ...
          return res.status(401).json({
            status: "failed",
            message: "failed to determine response type",
          });
        }
      } catch (err) {
        console.error(err);
        return res
          .status(401)
          .json({ status: "failed", message: "failed to authenticate" });
      }
      ...
      if (result.verified) {
        ...
        return res.status(200).json({
          ...
          user: {
            ...
            id: user.id,
            //callback: `exampleauth:////auth?token=${user.id}`,
            callback: `exampleauth:////auth?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_a&&%dQssw5c`,
          },
        });
      } else {
        return res
  ...
  authenticate(options = {}) {
    const { failureRedirect, usernameField = this.config.usernameField } =
      options;
    ...
    return async (req, res, next) => {
      if (!req.session) {
        next(
          new Error(
            "No session found. Did you set up express-session correctly?"
          )
        );
      }
  ...
  static verifyAuthenticatorAttestationResponse(webauthnResponse) {
    ...
    const authrDataStruct = Webauthn.parseMakeCredAuthData(
    ...
    if (ctapMakeCredResp.fmt === "fido-u2f") {
      ...
      const PEMCertificate = Webauthn.ASN1toPEM(
        ctapMakeCredResp.attStmt.x5c[0]
      );
      ...
      console.log("none safetynet request\n");
      ...
      console.log(
        "CLIENT_DATA_JSON ",
        base64url.decode(webauthnResponse.clientDataJSON)
      );
      ...
      throw new Error(
        `Unsupported attestation format: ${ctapMakeCredResp.fmt}`
      );
    }
  ...
  static verifyAuthenticatorAssertionResponse(webauthnResponse, authr) {
    ...
    const response = { verified: false };
    if (true) {
      const authrDataStruct =
        Webauthn.parseGetAssertAuthData(authenticatorData);
      console.log("AUTH_DATA", authrDataStruct);
